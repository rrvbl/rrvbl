...
async function loadTeamsIntoForms() {
  ...
  const selects = ['playerTeams2v2', 'playerTeams4v4', 'playerTeams5v5', 'transferNewTeam', 'matchTeamA', 'matchTeamB', 'matchWinner', 'teamLeague'];
  selects.forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.innerHTML = '';
    teams.forEach(team => {
      if(id.startsWith('playerTeams') && team.league !== id.slice(-3)) return;
...
playerForm?.addEventListener('submit', async e => {
  ...
  // Collect selected teams by league, optional
  let team_ids = [];
  ['playerTeams2v2', 'playerTeams4v4', 'playerTeams5v5'].forEach(id => {
    const sel = document.getElementById(id);
    if(sel && sel.value) team_ids.push(sel.value);
  });
...
matchForm?.addEventListener('submit', async e => {
  ...
  const sets = [];
  for(let i=1; i<=3; i++) {
    const winnerSelect = document.getElementById(`set${i}Winner`);
    const scoreInput = document.getElementById(`set${i}Score`);
    if(winnerSelect && scoreInput && winnerSelect.value && scoreInput.value) {
      sets.push({winner: winnerSelect.value, score: scoreInput.value});
    }
  }
  if(sets.length < 2) return alert('At least 2 sets required');
  const matchWinner = document.getElementById('matchWinner').value;
  const video_url = document.getElementById('matchVideo').value;

  // Format set_scores as JSON string
...
async function fetchLiveHome() {
  ...
  document.getElementById('matchScores').innerHTML = matches.map(m => `<div class='card'>${m.team_a.name} vs ${m.team_b.name}<br>Sets: ${JSON.parse(m.set_scores).map(s=> `${s.score} (Winner: ${m.winner.name})`).join(', ')}<br>Winner: ${m.winner.name}</div>`).join('');
}
...
async function renderTeamsPage() {
  const { data: teams } = await client.from('teams').select('*').order('name');
  document.getElementById('allTeams').innerHTML = teams.map(t => `<div class='card'><strong>${t.name}</strong><br>League: ${t.league}</div>`).join('');
}

// ✅ Render Transfers Page
async function renderTransfersPage() {
  const { data: transfers } = await client.from('transfers').select('*, players(name), old_team:teams(name), new_team:teams(name)').order('transferred_at', { ascending: false });
...
async function renderMatchesPage() {
  const { data: matches } = await client.from('matches').select('*, team_a:teams(name), team_b:teams(name), winner:teams(name)').order('played_at', { ascending: false });
  document.getElementById('allMatches').innerHTML = matches.map(m => {
    let sets = '';
    try {
      const parsedSets = JSON.parse(m.set_scores);
      sets = parsedSets.map((s,i) => `Set ${i+1}: ${s.score} (Winner: ${m.winner.name})`).join('<br>');
    } catch {
      sets = m.set_scores;
    }
    return `<div class='card'><strong>${m.team_a.name}</strong> vs <strong>${m.team_b.name}</strong><br>${sets}<br>Winner: ${m.winner.name}${m.video_url ? `<br><a href='${m.video_url}' target='_blank'>Match Video</a>` : ''}</div>`;
  }).join('');
}
